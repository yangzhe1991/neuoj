...
def index(request):
    ...
    u = User.objects.all()
    if len(u) == 0:
        u = User(username='admin', password='19910521', nickname='god', email='s', isManager=True, isBoy=True,
                 website='', school='NEU', AC=0, submit=0)
        u.save()
...
def status(request):
    ...
    if mc.get('results') != None and len(mc.get('results')) > 0:
        ...
        for s in ss:
            ...
            if submit.result == 'AC':
                if not s[0] and len(
                        Submition.objects.filter(user=submit.user, problem=submit.problem, result='AC')) == 0:
                    #raise Http404
                    submit.user.AC += 1
                    submit.user.save()
                submit.problem.AC += 1
...
def submit(request, problem):
    ...
    if request.method == 'POST':
        if not 'login' in request.session:
            error = "please login first"
            return render_to_response('ojsubmit.html', dict(context, **{'error': error}))
        s = Submition(user=request.session['login'], problem=p[0], source=request.POST['source'],
                      lang=request.POST['lang'], sourcelong=len(request.POST['source']))
        s.save()
...
def login(request):
    ...
    if 'username' in request.POST and 'password' in request.POST:
        li = Logininfo(username=request.POST['username'], password=request.POST['password'],
...
def user(request, username):
    ...
    return render_to_response('ojuser.html',
                              dict(context, **{'user': u, 'ACs': l, 'submits': l2, 'rank': rank, 'lastlogin': login}))
...
def loginhistory(request, username):
    ...
    return render_to_response('ojloginhistory.html',
                              dict(context, **{'logins': Logininfo.objects.filter(username=username)}))
...
def adduser(request):
    if request.method != 'POST':
        raise Http404
    u = User(username=request.POST['username'], password=request.POST['password'], nickname=request.POST['nickname'],
             isBoy=True, school=request.POST['school'])
