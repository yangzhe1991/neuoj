...
def getheader(request, con):
    ...
    if con.isPublic != 2:
        if 'login' in request.session:
            ...
            if len(u) == 0:
                uu = ContestUser(contest=con, username=request.session['login'].username,
...
def submit(request, cid, pid):
    ...
    if request.method == 'POST':
        p.submit = F('submit') + 1
        p.save()
        s = ContestSubmition(user=context['contestlogin'], problem=p, source=request.POST['source'],
    ...
    for cu in cus:
        ...
        for cs in css:
            ...
            if cs.result == 'AC':
                ...
                users[cs.user][1] += (cs.create - cs.user.contest.start).seconds / 60 - users[cs.user][2][pdict[p]][
                    'problem'] * 20
